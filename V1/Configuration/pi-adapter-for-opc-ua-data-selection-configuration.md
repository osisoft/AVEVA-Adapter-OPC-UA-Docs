...
Complete the following steps to configure an OPC UA data selection. Use the `PUT` method in conjunction with the `api/v1/configuration/<ComponentId>/DataSelection/<StreamId>` REST endpoint to initialize the configuration.
...
6. Enter the following cURL command (which uses the `PUT` method) to initialize the data selection configuration.
    ...
    * If you use a component ID other than `OpcUa1` and a stream ID other than `ns=5;s=Random1`, update the endpoint with your chosen component ID and stream ID.
