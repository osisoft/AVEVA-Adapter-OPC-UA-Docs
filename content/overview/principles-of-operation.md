...
### Cold
If the adapters are operating in cold mode, then the secondary adapter is configured but not started. Once a failover event occurs, the secondary will become primary and will then begin to collect and egress data.

### Warm 
When the adapters are configured in warm mode, the component is started and connected to the data source, but it is not collecting or egressing any data. Once a failover event occurs, then the secondary will become primary and begin to collect and egress data.
