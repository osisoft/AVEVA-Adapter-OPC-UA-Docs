 ...
 **StartTime** | `datetime` | Time when the first data items are collected.<br><br> **Note:** Timestamps are interpreted into the Coordinated Universal Time (UTC) time standard. OSIsoft recommends the following format for entering a startTime as UTC in a curl command: *YYYY-MM-DD*T*HH:MM:SS*Z. <br><br>**Note:** If a timestamp is not specified in UTC, the adapter assumes it is in local time and will interpret the timestamp to UTC. For example, a timestamp of "2022-05-23T11:00" will be interpreted as "2022-05-23T11:00-07:00" if the adapter is in PDT time (UTC -7:00).
 **EndTime** | `datetime`| Time when the last data items are collected.<br><br> **Note:** Timestamps are interpreted into the Coordinated Universal Time (UTC) time standard. OSIsoft recommends the following format for entering a endTime as UTC in a curl command: *YYYY-MM-DD*T*HH:MM:SS*Z. <br><br>**Note:** If a timestamp is not specified in UTC, the adapter assumes it is in local time and will interpret the timestamp to UTC. For example, a timestamp of "2022-05-25T12:00" will be interpreted as "2022-05-25T12:00-07:00" if the adapter is in PDT time (UTC -7:00).
